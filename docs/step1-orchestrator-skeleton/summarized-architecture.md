...
**4-Core CPU, 4–5GB RAM.** This dictates every design choice. Large models, heavy containers, and dense vector databases are physically impossible without thrashing.

### **2. Architectural Rules**
*   **Deterministic Orchestrator:** The control loop is a lightweight Python state machine, *not* an LLM. LLMs are merely hot-swapped tools.
*   **Hot-Swapping:** Load Model A → Generate → Save to disk → Unload → Load Model B. KV caches cannot be shared between different models; only text artifacts are passed.
*   **State on Disk:** Use SQLite for runtime state (node status, attempts) and the filesystem for artifacts (code, logs, contracts).

### **3. Model Stack (Quantized GGUF)**
*   **Planner/Research:** `Qwen2.5-1.5B-Instruct` (Q4_K_M or Q5_K_M). Handles query decomposition and research. *Do not use 2/3-bit; it destroys JSON/logic reliability.*
*   **Coder:** `Qwen2.5-Coder-1.5B-Instruct` (Q4_K_M). Handles code/test generation.
*   **Optional Judge:** `Qwen2.5-0.5B-Instruct` (Q4_K_M). Only for lightweight policy triage, not primary fact-checking.

### **4. The Execution Loop**
1. **Plan:** Planner generates a strict, immutable **JSON DAG**. Each node must define: ID, dependencies, inputs/outputs, acceptance criteria, and sandbox rules. (Max 24 nodes, no cycles).
2. **Research:** Planner queries SearXNG/local BM25, extracts atomic claims, scores them with a tiny reranker, and saves a text-based **Knowledge Brief**. (Defer heavy Knowledge Graphs).
3. **Code (TDD):** Coder loads. Uses a **dependency-aware sliding window** (passes interface *contracts*, not full previous code). Generates test + code.
4. **Validate:** Runs in a lightweight **`bubblewrap` sandbox** (no network, strict file limits, hard timeout). 
5. **Retry:** Max **3 attempts** per node. If it fails, log error, increment attempt, and retry. If 3 fails, mark `FAILED` and halt.

### **5. Resource Throttling (Dynamic Bifurcation)**
If CPU > 80% or RAM < 800MB, the orchestrator must:
*   Shrink the context window (target **2048 tokens** for 4GB, **3072** for 5GB).
*   Truncate knowledge briefs and sliding windows.
*   **Split** the current node into smaller sub-nodes (max split depth: 3).

### **6. Open Decisions Required From You**
To finalize the plan, confirm these 6 points:
1. **OS:** Is the target device Linux? (Required for `bubblewrap`).
2. **Context Size:** Default to 2048 or 3072 tokens?
3. **Runtimes:** Are Python, GCC, and Node.js pre-installed on the device?
4. **Internet:** Online (SearXNG) or Offline (local SQLite FTS5 keyword search)?
5. **Attempt Definition:** Are the 3 attempts *total* (including test generation failures), or 3 code fixes *after* a valid test exists? (Recommend *total* for safety).
6. **HTML/CSS Validation:** Static linting and DOM structure checks only (no heavy headless browser). Acceptable? 

Reply with your choices for the 6 open decisions, and the planning phase is complete.
